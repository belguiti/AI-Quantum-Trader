...
bot_path = r"c:\Users\azdin\.gemini\antigravity\scratch\ai-quantum-trader\frontend\src\app\components\bot-settings\bot-settings.component.ts"
...
# Special for Bot Settings
bot_content = re.sub(
    r'class="(.*?)glass-card(.*?)p-6(.*?)"',
    r'class="\1bg-white border border-slate-200 shadow-sm dark:bg-slate-800 dark:border-slate-700 dark:shadow-lg rounded-2xl p-6\3"',
    bot_content
)
bot_content = convert_global(bot_content)
with open(bot_path, 'w', encoding='utf-8') as f:
    f.write(bot_content)


ai_path = r"c:\Users\azdin\.gemini\antigravity\scratch\ai-quantum-trader\frontend\src\app\components\ai-signals\ai-signals.component.html"
with open(ai_path, 'r', encoding='utf-8') as f:
    ai_content = f.read()

ai_content = convert_global(ai_content)
