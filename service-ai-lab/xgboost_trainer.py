...
class ContinuousXGBoostTrainer:
    ...
    def backtest(self, symbol: str, df: pd.DataFrame, atr_multiplier: float = 1.5, start_date_filter: str = None) -> dict:
        ...
        sl_price: float  = 0.0
        tp_price: float  = 0.0
        position_fraction = 0.0  # fraction of equity deployed in current trade
        ...
        rm = RiskManager.from_warmup(
            closes[:warmup_n], capital,
            daily_loss_limit_pct=0.05,
            max_drawdown_pct=0.10,
            max_position_pct=0.20,       # never deploy > 20% of equity
        )
        ...
        for i in range(len(pred_proba)):
            ...
            if position_open:
                if position_side == "BUY":
                    if low <= sl_price:        # SL hit
                        ...
                        capital, fee = RiskManager.apply_pnl(
                            capital, position_fraction, pnl_pct, COMMISSION_RATE, SLIPPAGE_RATE)
                        total_fees += fee
                        losses += 1
            ...
            # ── Entry signals — guarded by risk limits ──
            if not position_open and atr_val > 0:
                ...
                if can_trade:
                    if prob_buy >= CONFIDENCE_THRESHOLD and prob_buy > prob_sell:
                        ...
                        capital, fee      = RiskManager.apply_entry_fee(
                            capital, position_fraction, COMMISSION_RATE)
                        total_fees       += fee
                        position_open     = True
                        position_side     = "BUY"
                        buy_trades       += 1
